...
class classDetailsStripe {
  ...
  viewClassLocations(data) {
    ...
    data[0].forEach((level) => {
      level.location.forEach((location) => {
		...
		locationOption.value = location.locationName.toLowerCase().replace(/\s/g, "-");
		locationOption.innerHTML = location.locationName
    locationOption.setAttribute("responseText", btoa(JSON.stringify(location)) )
    ...
    let paymentType = paymentTab.getAttribute('data-w-tab') == 'Tab 1' ? 'ach' : 'card'
    let has_fee = paymentType == 'card' ? true : false;
		$this.initializeStripePayment(submitClassPayment, responseText, timingText, selectField, paymentType, has_fee, classId, levelName);
	})
  }
  ...
  initializeStripePayment(
    ...
    var suppProIdE = document.getElementById('suppProIds');
		let upsellProgramIds = JSON.parse(suppProIdE.value).length > 0 ? JSON.parse(suppProIdE.value).map(Number) : [];
  ...
  displaySelectedSuppProgram(selectedIds) {
    ...
    let cartGridWrapper = creEl('div', 'cart-grid-wrapper')

		selectedData.forEach((sup) => {
			//var suppProDiv = creEl('div', 'horizontal-div align-left');
			let suppleHeadingDiv = creEl('div', 'prev_student_row horizontal-div')

			let offeringType = creEl("div", "main-text small-medium-mb-0");
			offeringType.innerHTML = sup.label;

			let offeringRemove = creEl("div", "main-text brown-red-text-small");
