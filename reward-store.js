  ...
  buildTabs() {
    ...
    this.$millions_transactions.forEach((student, idx) => {
      ...
      tabMenu += `
                            <a data-w-tab="Tab ${
                              idx + 1
                            }" class="portal-tab-link w-inline-block w-tab-link ${isActive}" id="w-tabs-0-data-w-tab-${idx}" href="#w-tabs-0-data-w-pane-${idx}" role="tab" aria-controls="w-tabs-0-data-w-pane-${idx}" aria-selected="${ariaSelected}" ${tabIndex}>
                                <div class="poppins-para portal-tab-text-semibold">${
                                  student.studentName
                                }</div>
                            </a>
                        `;
      tabContent += `
                            <div data-w-tab="Tab ${
                              idx + 1
                            }" class="portal-tab-pane no-padding w-tab-pane ${isPaneActive}" id="w-tabs-0-data-w-pane-${idx}" role="tabpanel" aria-labelledby="w-tabs-0-data-w-tab-${idx}" style="${
        isPaneActive
          ? "opacity: 1; transition: all, opacity 0.3s, opacity 300ms;"
          : ""
      }">
                                <div class="portal-flex-wrapper reward">
                                    <div class="million-price-text reward-balance">
                                        <span class="reward-balance-text">Your Balance:</span> ${this.formatMillions(
                                          student.earnAmount
                                        )}M
                                    </div>
                                </div>
      ...
      const itemHtml = `
                            <div class="portal-white-banner-reward">
                                ...
                                <div class="reward-card-info">
                                    <p class="dm-sans reward">${
                                      item.itemName || ""
                                    }</p>
                                    <p class="poppins-para">${
                                      item.description || ""
                                    }</p>
                                    <div class="reward-cost-wrapper">
                                        <div class="portal-flex-wrapper">
                                            <div class="million-price-text reward">${this.formatMillions(
                                              item.amount
                                            )} <span class="million-text-gray reward">million</span></div>
                                        </div>
                                        <a href="#" class="main-button white-bold-rounded w-button">Buy Now</a>
                                    </div>
                                </div>
                            </div>
                        `;
