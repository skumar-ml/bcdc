  ...
  class classDetailsStripe {
    ...
    async validateUserForProgram() {
      ...
      const data = await this.fetchData(
        "getAllPreviousStudents/" + this.webflowMemberId + "/current"
      );
      if (data == "No data Found") {
        return { eligible: false };
      } else {
        // toLowerCase() and trim the first and last name before compare and remvoe extra space between names
        
        data = data.filter(item => item.isSummerProgram == false && item.studentName.toLowerCase().trim().replace(/\s+/g, ' ') == (studentFirstName.value+studentLastName.value).toLowerCase().trim().replace(/\s+/g, ' '));
        if (data.length > 0) {
