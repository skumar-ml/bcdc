...
class classDetailsStripe extends parentLogin {
  ...
  constructor(
    ...
    // successful Stripe payment (referrer = /payment-confirmation). Must run
    // before any localStorage reads so the form doesn't prefill old data.
  ...
  async renderCheckoutData(memberId) {
    try {
      ...
      try {
        var suppData = await this.fetchData("getUpsellProgramOne?session=abc", this.typeEBaseUrl);
  ...
  addToCart() {
    ...
    document.addEventListener("click", function (event) {
      ...
      if (isWineRedButton) {
        ...
        const semesterBundleModalWine = document.getElementById("semester-bundle-modal");
        $this.closeModal(semesterBundleModalWine);
        window.scrollTo({ top: 0, behavior: "smooth" });

        let paymentTabWine = document.querySelectorAll(".payment-cards-tab-link");
        if (paymentTabWine.length > 0) {
          paymentTabWine[0].click();
        }
        $this.updateAmount(0);
        $this.hideShowNewStudentFee("none");
        $this.disableEnableBuyNowButton();
