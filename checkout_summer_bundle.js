...
class CheckOutWebflow {
	...
	_parseProgramDiscAmount(program) {
      if (!program) {
        return 0;
      }
      var raw = program.disc_amount != null && program.disc_amount !== ""
        ? program.disc_amount
        : program._baseAmount;
      return this.trimToTwoDecimals(raw);
    }

	hideShowNewStudentFee(type) {
      var bundleOrderSummary = document.querySelectorAll(".bundle-order-details-new-div");
      var defaultOrderSummary = document.querySelectorAll(".bundle-order-details-old-div");
      this.updateDepositePriceForBundle();
      if (this._hasAnyUpsellSelected()) {
        defaultOrderSummary.forEach(function (el) { el.style.display = "none"; });
        bundleOrderSummary.forEach(function (el) { el.style.display = "block"; });
      } else {
        defaultOrderSummary.forEach(function (el) { el.style.display = "block"; });
        bundleOrderSummary.forEach(function (el) { el.style.display = "none"; });
      }
    }

	updateDepositePriceForBundle() {
      var defaultOrderSummaries = document.querySelectorAll(".bundle-order-details-old-div");
	...
	updateOriginPrice() {
      var container = document.querySelectorAll('[data-upsell="original-tution-fee"]');
      ...
      var coreId = this.$coreData && this.$coreData.upsellProgramId;
      var programs = [];
      var core = (this.$selectedProgram || []).find(function (p) {
      ...
      (this.$selectedProgram || []).forEach((program) => {
        if (
          program &&
          program.upsellProgramId !== coreId &&
          !programs.some(function (p) { return p.upsellProgramId === program.upsellProgramId; })
        ) {
      ...
      programs.forEach(function (program) {
        var grid = creEl("div", "cart-grid-wrapper");
        var label = creEl("p", "main-text order-details");
        label.textContent = program.label ? ("Tuition " + program.label) : "Tuition";
        var priceWrap = creEl("div", "w-embed");
        var price = creEl("p", "main-text order-details-price");
        var lineAmount = $this._parseProgramDiscAmount(program);
        price.textContent = "$" + $this.numberWithCommas($this.trimToTwoDecimals(lineAmount));
        ...
        tuitionTotal += lineAmount;
      });

      var totalGrid = creEl("div", "cart-grid-wrapper");
      var totalLabel = creEl("p", "main-text order-details");
      totalLabel.textContent = "Tuition Total";
      var totalWrap = creEl("div", "w-embed");
      var totalPrice = creEl("p", "main-text order-details-price");
      ...
      var feeGrid = creEl("div", "cart-grid-wrapper");
      var feeLabel = creEl("p", "main-text order-details");
      feeLabel.textContent = "New Student Fee";
      var feeWrap = creEl("div", "w-embed");
      var feePrice = creEl("p", "main-text order-details-price");
   ...
   displaySelectedSuppPrograms(selectedData, selectedSuppPro) {
	...
	async displaySupplementaryProgram() {
		try {
          var suppData = await this.fetchData("getUpsellProgram?session=summer", this.memberData.eTypeBaseUrl);
          ...
          console.error("Error fetching getUpsellProgram (summer):", error);
