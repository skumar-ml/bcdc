class Portal {
  ...
  setupTabs(data, millionsData, announcements) {
    ...
    students.forEach((student, idx) => {
      ...
      const currentSession =
        studentData.currentSession && studentData.currentSession.length > 0
          ? studentData.currentSession[studentData.currentSession.length - 1]
          : null;
      let tabLink, tabPane;
      if (idx === 0) {
      ...
      if (currentSession) {
        ...
        tabPane
          .querySelectorAll(
            '.recent-announcement-div.current-class, .recent-announcement-info-div, [data-portal="invoice-form-accordian"], .calendar-info-grid-wrapper, .class-tools-quick-links-div, .millions-balance-flex-wrapper'
          )
          .forEach((el) => {
            if (el) el.style.display = "";
          });
        // Show/hide registration-form-accordian based on formList in currentSession
        const regFormAccordian = tabPane.querySelector(
          ".registration-form-accordian"
        );
        if (
          currentSession.formList &&
          Array.isArray(currentSession.formList) &&
          currentSession.formList.length > 0
        ) {
        ...
        tabPane
          .querySelectorAll(".registration-form-accordian")
          .forEach((el) => {
            if (el) el.style.display = "none";
          });
      }
      ...
      // Past Class History
      const pastClassesDiv = tabPane.querySelectorAll(
        ".sem-classes-info-div"
      )[1]; // assuming second div is for past classes
      if (
        studentData.pastSession &&
        Array.isArray(studentData.pastSession) &&
        studentData.pastSession.length > 0
      ) {
  ...
  renderStudentTab(tabPane, student, millionsData, announcements) {
    ...
    const currentClassDiv = tabPane.querySelector(
      ".recent-announcement-div.current-class"
    );
    if (currentClassDiv) {
      let hasClassDetail =
        student.classDetail && Object.keys(student.classDetail).length > 0;
      if (hasClassDetail) {
        ...
        // Update title
        const titleEl = currentClassDiv.querySelector(
        ...
        // Update class info
        const classInfoEl = currentClassDiv.querySelector(
          ".poppins-para.current-class"
        );
        if (classInfoEl) {
          classInfoEl.textContent = `${classLevel} | ${day} ${startTime} | ${location}`;
        }
      } else if (
        student.summerProgramDetail &&
        Object.keys(student.summerProgramDetail).length > 0
      ) {
    ...
    if (Array.isArray(millionsData)) {
      const millionsEntry = millionsData.find(
        (entry) => entry.studentName === student.studentDetail.studentName
      );
      if (millionsEntry) {
    ...
    if (
      student.futureSession &&
      Array.isArray(student.futureSession) &&
      student.futureSession.length > 0
    ) {
      if (futureClassesDiv) futureClassesDiv.style.display = "";
    ...
    if (
      student.pastSession &&
      Array.isArray(student.pastSession) &&
      student.pastSession.length > 0
    ) {
      if (pastClassesDiv) pastClassesDiv.style.display = "";
      // Optionally, update the content if needed
    } else {
    ...
    if (calendarDiv) {
      const googleCalendar = (student.uploadedContent || []).find(
        (u) => u.label === "Google Calendar"
      );
      let calendarLink = "";
      if (
        googleCalendar &&
        googleCalendar.upload_content &&
        googleCalendar.upload_content.length > 0
      ) {
        calendarLink = googleCalendar.upload_content[0].link;
      }
      if (calendarLink) {
        calendarDiv.innerHTML =
          '<p class="portal-node-title">Calendar Graph</p>';
        ...
        calendarDiv.innerHTML =
          '<p class="portal-node-title">Calendar Graph</p><div>No records available</div>';
      }
    }
    // Schedule a Makeup (Make-up Acuity Link)
    const makeupDiv = tabPane.querySelector(".class-tools-quick-links-div");
    if (makeupDiv) {
      const makeupLink = (student.uploadedContent || []).find(
        (u) => u.label === "Make-up Acuity Link"
      );
      const makeupSection = makeupDiv.querySelector(
        ".class-tools-quick-links-flex-wrapper"
      );
      if (makeupSection) {
      ...
      if (
        makeupLink &&
        makeupLink.upload_content &&
        makeupLink.upload_content.length > 0
      ) {
        if (makeupSection) {
          makeupLink.upload_content.forEach((item) => {
            ...
            icon.loading = "lazy";
            icon.src =
              "https://cdn.prod.website-files.com/64091ce7166e6d5fb836545e/68752684beabb8d0a43428b5_calendar_today.svg";
            icon.alt = "";
            ...
            img.src =
              "https://cdn.prod.website-files.com/64091ce7166e6d5fb836545e/6877c5c978d548ccecec33bf_link.svg";
            img.alt = "Copy link";
    ...
    if (zoomDiv) {
      const zoomLinks = (student.uploadedContent || []).find(
        (u) => u.label === "Zoom links"
      );
      const zoomSection = zoomDiv.querySelector(".zoom-links-info-div");
      if (
        zoomLinks &&
        zoomLinks.upload_content &&
        zoomLinks.upload_content.length > 0
      ) {
        // Remove all previous .zoom-links-info-div children
        zoomDiv
          .querySelectorAll(".zoom-links-info-div")
        ...
        if (zoomSection)
          zoomSection.innerHTML = "<div>No records available</div>";
      }
    }
    // General Resources
    const generalDiv = tabPane.querySelector(".general-resources-info-wrapper");
    if (generalDiv) {
      const generalResources = (student.uploadedContent || []).find(
        (u) => u.label === "General resources"
      );
      // Remove all previous .general-resources-flex-wrapper children
      generalDiv
        .querySelectorAll(".general-resources-flex-wrapper")
        .forEach((el) => el.remove());
      if (
        generalResources &&
        generalResources.upload_content &&
        generalResources.upload_content.length > 0
      ) {
        ...
        noDiv.innerHTML =
          '<div><div class="dm-sans title"><span>No records available</span></div></div>';
        generalDiv.appendChild(noDiv);
      }
    }
    // Recent Announcements
    const announcementDiv = tabPane.querySelector(
      ".recent-announcement-info-div"
    );
    if (
      announcementDiv &&
      Array.isArray(announcements.announcement) &&
      announcements.announcement.length > 0
    ) {
      // Show count
      ...
      announcementDiv
        .querySelectorAll(
          ".recent-announcement-info-inner-div, .recent-announcement-info-flex, .dm-sans.recent-announcement-info"
        )
        .forEach((el) => el.remove());
      recent.forEach((ann) => {
  ...
  renderGoogleCalendar(tabPane, student) {
    ...
    if (
      ...
      const calendarEmail = encodeURIComponent(
        student.studentDetail.parentEmail
      );
  ...
  renderHomeworkLink(tabPane, student) {
    ...
    const homeworkImg = homeworkDiv
      ? homeworkDiv.querySelector('[data-portal="homework-link-image"]')
      : null;
    if (homeworkDiv) {
      if (student.studentDetail.home_work_link) {
        ...
        if (homeworkImg && !homeworkImg._copyHandlerAttached) {
          ...
          homeworkImg.addEventListener("mouseenter", () =>
            showTooltip("Copy homework link")
          );
          homeworkImg.addEventListener("mouseleave", hideTooltip);
          homeworkImg.addEventListener("focus", () =>
            showTooltip("Copy homework link")
          );
          homeworkImg.addEventListener("blur", hideTooltip);
          homeworkImg.addEventListener("click", () => {
            navigator.clipboard
              .writeText(student.studentDetail.home_work_link)
              .then(() => {
                showTooltip("Copied!");
                setTimeout(hideTooltip, 1200);
              });
  ...
  renderInvoiceAccordionStyled(tabPane, student) {
    ...
    const invoiceAccordion = tabPane.querySelector(
      '[data-portal="invoice-form-accordian"]'
    );
    ...
    // Accordion header
    invoiceAccordion.className =
      "invoice-form-accordian registration-form-accordian"; // reuse styles
    invoiceAccordion.innerHTML = `
                    <div class="registration-form-accordion-header">
                        ...
                        <div class="registration-form-flex-wrapper">
                            <div class="registration-form-tag">
                                <p class="dashboard-tag-text">${
                                  invoices.some((inv) => !inv.is_completed)
    ...
    const body = invoiceAccordion.querySelector(
      ".registration-form-accordion-body"
    );
    ...
    // Progress text
    const progressText = body.querySelector(
      ".medium-text-with-margin-auto span"
    );
    if (progressText)
      progressText.textContent = `${completed} of ${total} invoices complete`;
    ...
    invoices.forEach((inv) => {
      ...
      div.className =
        "required-forms-flex-wrapper bluish-gray-rounded-with-mt-10";
      const isCompleted = inv.is_completed;
      let actionLinks = "";
      if (!isCompleted && Array.isArray(inv.jotFormUrlLink)) {
        actionLinks = inv.jotFormUrlLink
          .map((link) => {
            if (
      ...
      div.innerHTML = `
                        <div class=\"required-forms-inner-flex-wrapper\">
                            ...
                            <div>
                                <div class=\"poppins-para medium\"><span>${
                                  inv.invoiceName
                                }</span></div>
                                <div class=\"poppins-para dark-gray\"><span>${
                                  inv.status === "Complete" || isCompleted
                                    ? "Paid"
                                    : "Pending"
                                }</span></div>
                            </div>
                        ...
                        <div class=\"required-forms-tags-wrapper\">
                            <div class=\"${
                              isCompleted
                                ? "required-form-completed-tag"
                                : "required-form-in-completed-tag"
                            }\">
                                <p class=\"${
                                  isCompleted
                                    ? "completed-tag-text"
                                    : "incompleted-tag-text"
                                }\">${
    ...
    const header = invoiceAccordion.querySelector(
      ".registration-form-accordion-header"
    );
    header.addEventListener("click", function () {
      if (invoiceAccordion.classList.contains("open")) {
        ...
        tabPane
          .querySelectorAll(".registration-form-accordian.open")
          .forEach((faq) => {
  ...
  renderFailedInvoiceNotification(tabPane, student) {
    if (
      student.invoiceList &&
      student.invoiceList.some((inv) => inv.status === "Failed")
    ) {
      const failedDiv = document.createElement("div");
      failedDiv.className = "notification-failed-invoice";
      failedDiv.innerHTML =
        "<span>Some invoices have failed payments. Please pay again.</span>";
  ...
  renderRegistrationForms(tabPane, student) {
    ...
    if (regFormAccordian) {
      const regFormBody = regFormAccordian.querySelector(
        ".registration-form-accordion-body"
      );
      if (regFormBody) {
        const requiredFormsDiv = regFormBody.querySelector(
          ".required-forms-div"
        );
        const progressDiv = regFormBody.querySelector(
          ".completion-progress-div"
        );
        const needsToBeCompleted = regFormAccordian.querySelector(
          ".dashboard-tag-text"
        );
        if (requiredFormsDiv) {
          ...
          const regFormTitle = regFormAccordian.querySelector(
            ".registration-form-title span"
          );
          if (
            regFormTitle &&
            regFormTitle.textContent.trim() === "Registration Forms"
          ) {
            if (forms.length === 0) {
          ...
          const checkform = (formId) => {
            if (!formId || !Array.isArray(student.formCompletedList))
              return false;
          ...
          if (forms.length === 0) {
            ...
            noFormDiv.className =
              "required-forms-flex-wrapper bluish-gray-rounded";
            ...
            const progressText = regFormBody.querySelector(
              ".medium-text-with-margin-auto span"
            );
            if (progressText) {
              progressText.textContent = `${completedForms} of ${totalForms} forms complete`;
            }
            const percentText = regFormBody.querySelector(
              ".bold-text-with-margin-auto span"
            );
            if (percentText) {
              percentText.textContent = `${percent}%`;
            }
            const progressBar = regFormBody.querySelector(
            ...
            forms.forEach((form) => {
              ...
              if (is_live) {
                if (editable) {
                  ...
                  let dbData = (student.formCompletedList || []).find(
                    (o) => o.formId == form.formId
                  );
                  if (form.is_editable) {
                    formLinkHref =
                      form.formId && dbData && dbData.submissionId
                        ? `https://www.jotform.com/edit/${
                            dbData.submissionId
                          }?memberId=${
                            student.uniqueIdentification || ""
                          }&classId=${
                            student.classDetail
                          ...
                          )}&accountEmail=${encodeURIComponent(
                            student.studentDetail.parentEmail
                          )}&paymentId=${
                            student.studentDetail.uniqueIdentification
                          }`
                        : "";
                    actionText = "Edit form";
                  } else {
                    formLinkHref =
                      dbData && dbData.submissionId
                  ...
                  formLinkHref = form.formId
                    ? `https://form.jotform.com/${form.formId}?memberId=${
                        ...
                        student.classDetail ? student.classDetail.classId : ""
                      }&studentName=${encodeURIComponent(
                        student.studentDetail.studentName
                      )}&accountEmail=${encodeURIComponent(
                        student.studentDetail.parentEmail
              ...
              const linkClass =
                is_live && window.innerWidth > 1200
                  ? "iframe-lightbox-link dm-sans edit-form"
                  : "dm-sans edit-form";
  ...
  initStripePaymentLinks() {
    setTimeout(() => {
      document
        .querySelectorAll(".pay-link[data-invoice-id]")
        .forEach((link) => {
